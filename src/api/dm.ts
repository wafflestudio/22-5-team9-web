...
export const dmApi = {
  ...
  async getConversationMessages({ recipientId, offset, limit }: GetConversationMessagesParams): Promise<Message[]> {
    const response = await fetch(`/api/messages/conversation/${recipientId}?offset=${offset}&limit=${limit}`);
    if (!response.ok) {
